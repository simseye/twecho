...
def create_urls_new():
    ...
    cnx.cmd_query('create table if not exists urls_tid_new(\
                    tweet_id bigint,\
                    url_name varchar(25),\
                    index_begin smallint,\
                    index_end smallint,\
...
def create_media():
    cnx.cmd_query("create table if not exists media(\
                ...
                met_playback_0 int unsigned,\
                met_playback_25 int unsigned,\
                met_playback_50 int unsigned,\
                met_playback_75 int unsigned,\
                met_playback_100 int unsigned,\
...
def create_place_tweet():
    cnx.cmd_query("create table if not exists place_tweet(\
                    tweet_id bigint,\
                    place_id varchar(20),\
                    foreign key(tweet_id) references tweets(id),\
...
def create_list_tags_users():
    cnx.cmd_query("create table if not exists list_tags_users(\
                    user_id bigint,\
                    tag_name varchar(45),\
...
def create_context_annotations():
    cnx.cmd_query("create table if not exists context_annotations(\
...
def create_context_entities():
    cnx.cmd_query("create table if not exists context_entities(\
                    id bigint unsigned,\
                    name varchar(200),\
                    primary key (id)\
                        )")
...
def create_context_tweets():
    cnx.cmd_query("create table if not exists context_tweets(\
                    tweet_id bigint,\
                    annotation_id smallint unsigned,\
...
def insert_users_v2(includes):
    ...
    for user in includes['users']:
        tweet_dtu = datetime.strptime(user['created_at'], '%Y-%m-%dT%l:%M:%S.%fZ')
        ddb_dtu = tweet_dtu.strftime("%Y-%b-%d %H:%M:%S")
        ddb_dtu = ddb_dtu[:5] +  str(tweet_dtu.month) + ddb_dtu[8:]
        instr_user= \
            "insert ignore into  users(\
...
def insert_urls_new(tweets):
    for i, tweet in enumerate(tweets):
        ...
        for url in tweet['entities'].get('urls', []):
            ...
            url_tid_string =\
            "insert ignore into urls_tid_new( \
            tweet_id, \
            url_name, \
            index_begin,\
            index_end)\
...
def insert_list_tags_users(users_dic, list_name):
    for user in users_dic:
        qt_tweet_s=\
            "insert ignore into  list_tags_users(\
            user_id ,\
            tag_name \
            )\
            values(%s, %s)"
...
def insert_context_annotations(tweets):
    for tweet in tweets: 
        ...
        for anno in cas:
            query =\
                "insert ignore into context_annotations(\
                id,\
                name,\
                description\
                )\
            ...
            query =\
                "insert ignore into context_entities(\
                id,\
                name)\
                values (%s, %s)"
            values = (
            ...
            query =\
                "insert ignore into context_tweets(\
                    tweet_id,\
                    annotation_id,\
                    entity_id)\
                    values( %s, %s, %s)"
...
def load_user_json(j_array):
    ...
    cursor.execute(f"select * from tweets where id = {int(j_array[0]['id_str'])}")
...
def initialize_db():
    ...
    # connect args -> user=user, passwd=password, host=host, database=database_name, host=host
    cnx = mysql.connector.connect(**s.db_args)
...
def create_index():
    cursor.execute("create index retweeted_ids on tweets(retweeted_id)")
    cnx.commit()
    cursor.execute("create index quoted_ids on tweets(quoted_status_id)")
    cnx.commit()
    cursor.execute("create index replied_to_ids on tweets(in_reply_to_status_id)")
    cnx.commmit()
    cursor.execute("create index conversation_ids on tweets(conversation_id)")
    cnx.commit()
    cursor.execute("create index place_ids on tweets(place_id)")
    cnx.commit()
